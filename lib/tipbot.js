...
var TipBot = function(slack, BLOCKTRAIL_APIKEY, BLOCKTRAIL_APISECRET, SECRET, TESTNET, OPTIONS) {
    ...
    self.explorerBaseUrl = "https://www.blocktrail.com/" + (TESTNET ? "tBTC" : "BCC");
    ...
    self.CURRENCY_REGEX = new RegExp('(Satoshis?|BTC|' + self.CURRENCIES.join("|") + ')', "ig");
    self.AMOUNT_REGEX = new RegExp('(\\d+\\.?\\d*) *(Satoshis?|BCC|' + self.CURRENCIES.join("|") + ')', "i");
    ...
    self.getPriceRates(function(err, rates) {
        ...
        rates.forEach(function(rate) {
            if (BLACKLIST_CURRENCIES.indexOf(rate) === -1) {
                self.CURRENCIES.push(rate.code);
            }
        });

        self.CURRENCY_REGEX = new RegExp('(Satoshis?|BCC|' + self.CURRENCIES.join("|") + ')', "ig");
        self.AMOUNT_REGEX = new RegExp('(\\d+\\.?\\d*) *(Satoshis?|BCC|' + self.CURRENCIES.join("|") + ')', "i");
    });
...
TipBot.prototype._getPriceRates = function(filename, cb) {
    ...
    fs.exists(filename, function(exists) {
        if (exists) {
            ...
            request.get("https://bitpay.com/api/rates", function(err, response, body) {
    ...
    if (currency != 'BCC') {
        self.getPriceRates(function(err, rates) {
            ...
            if (!rate) {
                ...
                var newValue = Math.ceil(value / rate.rate * 1e8);

                var text = value.toFixed(2) + " " + currency + " " +
                    "(" + blocktrail.toBCC(newValue) + " BTC at " + rate.rate.toFixed(2) + " " + currency + " / BTC)";

                return cb(newValue, rate.rate, currency, value, text);
            }
        });
    } else {
        return cb(value, null, null, null, blocktrail.toBCC(value) + " BCC");
    }
};
        ...
        if (amount && message.match(/(send|give|sent)/i)) {
            self.normalizeValue(amount[1], amount[2], function(value, rate, originalCurrency, originalValue, valueText) {
                if (rate === false) {
                    ...
                    if (value < MIN_TIP) {
                        channel.send(user.handle + ": the minimum tip amount is " + blocktrail.toBCC(MIN_TIP) + " BCC");
                    ...
                    self.normalizeValue(amount[1], amount[2], function(value, rate, originalCurrency, originalValue, valueText) {
                        if (rate === false) {
                            ...
                            if (currency == 'BCC') {
                                ...
                                channel.send(blocktrail.toBCC(value) + " BCC (" + rate.toFixed(2) + " " + amount[2] + " / BCC)");
                            } else if (amount[2] == 'BCC') {
                                self.getPriceRates(function(err, rates) {
                                    ...
                                    if (!rate) {
                                        ...
                                        var newValue = blocktrail.toBCC(value) * rate.rate;

                                        channel.send(newValue.toFixed(2) + " " + currency + " (" + rate.rate.toFixed(2) + " " + currency + " / BTC)");
                                    }
                                });
                            } else {
                                self.getPriceRates(function(err, rates) {
                                    var currencyRate = _.find(rates, {code: currency});

                                    if (!currencyRate) {
                                        channel.send(user.handle + ": we don't support that currency yet!");
                                    } else {
                                        // for X to Y (non BTC) we use BTC to convert as in between step
                                        var conversionRate = currencyRate.rate / rate;
                                        var newValue = blocktrail.toBCC(value) * currencyRate.rate;
        ...
        if (currency) {
            ...
            self.getPriceRates(function(err, rates) {
                ...
                if (!rate) {
                    ...
                    channel.send("1.0 BCC is " + rate.rate.toFixed(2) + " " + currency);
                }
            });
